#!/usr/bin/env python3
"""
Cartoon mini-movie generator — 3 стікмен персонажі з анімацією ходьби
...
# Конфіг персонажів: 0=Остап(синій), 1=Поліна(рожевий), 2=Микола(зелений)
CHAR_CFG = [
    {'jacket': (55, 115, 225), 'tie': (200, 20, 20),  'voice': 'uk-UA-OstapNeural'},
    {'jacket': (220, 80, 150), 'tie': (255, 220, 50), 'voice': 'uk-UA-PolinaNeural'},
    {'jacket': (50, 170, 80),  'tie': (255, 140, 0),  'voice': 'uk-UA-OstapNeural'},
]
...
# ─── Слоти позицій (залежно від кількості персонажів на сцені) ────────────────

def slot_positions(n):
    if n == 1: return [240]
    if n == 2: return [140, 340]
    return [90, 240, 390]
...
def draw_char(draw, fi, cx, char_id, walking=False, facing_right=True, talking=False):
    ...
    shoe = int(36 * S)
    r_sh = int(5 * S)
    draw.rounded_rectangle([lfoot[0]-shoe, lfoot[1]-4, lfoot[0]+shoe//3, lfoot[1]+9], radius=r_sh, fill=STICK_LINE)
    draw.rounded_rectangle([rfoot[0]-shoe//3, rfoot[1]-4, rfoot[0]+shoe, rfoot[1]+9], radius=r_sh, fill=STICK_LINE)

    # ── Голова ──
    draw.ellipse([cx-HEAD_RX, HEAD_CY-HEAD_RY, cx+HEAD_RX, HEAD_CY+HEAD_RY],
                 fill=(195,200,210), outline=STICK_LINE, width=LW)
    shift = 18 if facing_right else -18
...
def draw_bubble(draw, text, font, cx, slot_i, n_slots):
    ...
    draw.rounded_rectangle([bx1, by1, bx2, by2], radius=13, fill=BUBBLE_BG, outline=BUBBLE_BD, width=3)
...
def render_scene(scene_def, scene_idx, initial_chars, work_dir):
    ...
    exit_data  = {}  # char_id: (from_x, to_x, duration)
    ...
    try:
        for fi in range(total_frames):
            ...
            # Малюємо зліва направо (лівіші = "позаду")
            for char_id in sorted(present, key=lambda c: char_targets.get(c, 240)):
                ...
                # Не малюємо якщо за кадром
                if cx < -HEAD_RX * 2 or cx > W + HEAD_RX * 2:
                    continue

                talking = (char_id == talking_char)
                draw_char(draw, fi, int(cx), char_id,
                          walking=walking, facing_right=facing_right, talking=talking)

                if talking and bubble_text:
                    slot_i = sorted_present.index(char_id)
                    draw_bubble(draw, bubble_text, font, int(cx), slot_i, len(sorted_present))
...
def main():
    ...
    clips         = []
    current_chars = {}  # {char_id: x} — зберігається між сценами
